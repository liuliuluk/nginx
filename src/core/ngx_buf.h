...
struct ngx_buf_s {
    /*
    pos通常是用来告诉使用者本次应该从pos这个位置开始处理内存中的数，这样设置是因为同一个ngx_buf_t可能被多次反复处理。
    当然，pos的含义是由使用它的模块定义的
    */
    u_char          *pos;
    /*
    last通常表示有效的内容到此位置，注意，pos与last之间的内存是希望nginx处理的内容
    */
    u_char          *last;
    /*
    ...
    */
    ...
    而是再次建立一个ngx_buf_t结构体指向原内存，这样多个ngx_buf_t结构体指向了同一款内存，他们之间的关系就通过
    ...
    */
     ...
     */
    unsigned         memory:1;//标志位：为1时表示数据中内存中且这段内存不可用被修改

    /* the buf's content is mmap()ed and must not be changed */
    unsigned         mmap:1;//标志位：为1时表示这段内存是用mmap系统调用映射过来的，不可用被修改

    unsigned         recycled:1;//为1时表示可回收
    unsigned         in_file:1;//为1时表示这段缓冲区处理的实是文件而不是内存
    unsigned         flush:1;//为1时表示需要执行flush操作
    /*
    对于操作这块缓冲区时是否使用同步方式，须谨慎考虑，这可能会阻塞nginx进程，nginx中所有操作几乎都是异步的，这是他支持
    高并发的关键。有些框架代码在sync为1时可能会有阻塞的方式进行I/O操作，它的意义视使用它的nginx模块而定
    */
    unsigned         sync:1;
    /*
    标志位：表示是否是最后一块缓冲区，因为ngx_buf_t可以由ngx_chain_t链表串联起来，
    因此当last_buf为1时表示当前是最后一刻待处理的缓冲区
    */
    unsigned         last_buf:1;
    unsigned         last_in_chain:1;//表示是否是ngx_chain_t中的最后一块缓冲区

    unsigned         last_shadow:1;//表示是否是最后一个影子缓冲区，与shadow域配合使用，通常不建议使用
